...
当设置 `force_ocr=true` 参数时，系统将执行以下操作：

### 1. 智能检测绕过
即使PDF已包含有意义的文本，也会强制进行OCR处理。

### 2. 文本层处理（新增功能）
在强制OCR模式下，系统会：
1. **删除原始文本层**：将PDF每页渲染为高分辨率图像（300 DPI）
2. **保留图像层**：保持原始视觉效果
3. **嵌入OCR文本**：将OCR结果以不可见但可搜索的方式嵌入到新的文本层

### 3. 技术实现
使用PyMuPDF将PDF页面渲染为图像，然后重建为纯图像PDF，最后嵌入OCR文本：
- **分辨率**: 300 DPI（保持清晰度）
- **图像格式**: PNG（无损压缩）
- **文本嵌入**: 透明文字，完全不可见但可搜索
